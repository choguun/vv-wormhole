...
import { ConfirmedIcon } from "./Theme/ConfirmedIcon";
import { CrossIcon } from "./Theme/CrossIcon";
import { ExclaimIcon } from "./Theme/ExclaimIcon";
import { PendingIcon } from "./Theme/PendingIcon";
    ...
    >
      ...
      <div className="flex px-4 py-3">
        ...
        <div className="ml-2 grow">
          ...
          {blockExplorer && (
            <Link
              className="text-ss-text-light hover:text-[#2E2D27] underline"
...
const TransactionSummary = ({
  ...
  const [timeUntilDismiss, setTimeUntilDismiss] = useState(
    AUTOMATIC_DISMISS_TIME
  );
  ...
  useEffect(() => {
    if (timeUntilDismiss <= 0)
      setDismissedTransactions([...dismissedTransactions, txEntity]);
  });
...
export function Transactions() {
  ...
  const visibleTransactions = allTransactions.filter(
    (t) => !dismissedTransactions.includes(t)
  );
  ...
  return (
    visibleTransactions.length > 0 && (
      <div
        style={{ zIndex: 100 }}
        className="absolute top-[8px] left-1/2 -translate-x-1/2 my-8 w-96"
      >
