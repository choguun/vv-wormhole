    ...
    <Dialog.Root open={isOpen}>
      ...
      <Dialog.Portal>
        ...
        >
          <Dialog.Content
            style={{
              width: small ? "600px" : "840px",
              maxHeight: "calc(100vh - 120px)",
            }}
            className="mx-auto DialogContent"
          >
            <Card className="relative w-full p-0">
              <div className="absolute top-0 left-0 flex flex-row justify-between items-center bg-white w-full pt-8 p-6 pb-4 border-b border-ss-stroke z-50 text-center">
                <Dialog.Title asChild>
                  <OverlineLarge className="mx-auto">{title}</OverlineLarge>
                </Dialog.Title>

                <div className="w-10" />

                {(isOpen && !setOpen) || noClose ? (
                  <></>
                ) : (
                  <Dialog.Close
                    onClick={() => {
