...
export const Button = React.forwardRef<HTMLButtonElement, Props>(
  (
    { type, className, pending, buttonType, disabled, children, ...props },
    ref
  ) => (
    <button
      ref={ref}
      ...
      className={twMerge(
        ...
        ButtonType.Primary === buttonType &&
          "bg-[#E6D089] text-black/60 hover:bg-[#EDC645]",
        ButtonType.Secondary === buttonType &&
          "bg-black-10 text-black/60 hover:bg-black/20",
        ButtonType.Confirm === buttonType &&
          "bg-[#61B258] text-black/60 hover:bg-[#4FAA4C]",
        ButtonType.Cancel === buttonType &&
          "bg-[#D9A3A3] text-black/60 hover:bg-[#C47B7B]",
        className
      )}
      {...props}
    >
